...
Substituting it back to the first expression, the characteristic equation can then be derived as
...
\subsection{Guaranteed Energy Dissipation}
The choice $\by_j=\mb{K}\bu$ yields a dynamic stiffness proportional to the exact stiffness matrix $\mb{K}$, enabling the definition of a scalar loss ratio and, potentially, an equivalent damping factor.
However, for systems exhibiting stiffness degradation or damage induced softening, this formulation may inject energy into the system rather than dissipate it.

To ensure a dissipative system, repeating this procedure with $\by_j=\bu$ gives the following stiffnesses:
\begin{gather}
    \mb{K}'=\mb{K}+\sum\dfrac{s_jm_j}{s_j^2+\omega^2}\mb{I},\qquad
    \mb{K}''=-\sum\dfrac{m_j\omega}{s_j^2+\omega^2}\mb{I}.
\end{gather}
Under this choice, the loss stiffness $\mb{K}''$ becomes independent of the static stiffness $\mb{K}$.
Although a scalar loss ratio can no longer be defined, the formulation guarantees positive energy dissipation.
...
\begin{gather}
    ...
    \end{split}
\end{gather}
Assuming the matrix $\diag{\mb{s}}+\mb{m}\mb{1}^\mT$ is neither defective nor degenerate, which is always possible by carefully selecting parameters $s_j$ and $m_j$, and denoting its eigenvalues as $s'_j$ and the corresponding left and right eigenvectors as $\mb{\phi}^l_j$ and $\mb{\phi}^r_j$, then
\begin{gather}
...
For forced vibration, the external load $\bp$ must be properly transformed to ensure the same equivalence, see later discussions.
\subsection{Wrong Asymptotic Behaviour}\label{sec:fix}
The fundamental consistency requirement states that $\mb{K}^*$ (or $\mb{K}'$) must approach the static stiffness $\mb{K}$ at low frequencies, that is, $\lim\limits_{\omega\rightarrow0}\mb{K}^*=\mb{K}$, to ensure the system behaves as expected under (quasi-)static loading conditions.
...
It is equivalent to \eqsref{eq:u_right} after a simple parameter mapping: $s_j$ stays unchanged, and replacing $m_j$ in \eqsref{eq:vel_formulation} by $-m_j/s_j$ yields \eqsref{eq:u_right}.
By comparing \eqsref{eq:u_right} and \eqsref{eq:vel_formulation}, one can also derive the following third identical formulation.
\begin{gather}\label{eq:vel_formulation_alter}
\left\{\begin{array}{l}
\mb{M}\ddot{\bu}+\mb{K}\bu-\sum\dfrac{\dot{\bbf}_j}{s_j}=\mb{0}, \\
\dot{\bbf}_j=-s_j\bbf_j+m_j\mb{K}\bu.
\end{array}\right.
\end{gather}
A careful comparison reveals that \eqsref{eq:vel_formulation_alter} is equivalent to the model by \citet{Huang2019}, which adopts real parameters $s_j$ and $m_j$.
One shall see that in essence this formulation applies an energy-dissipating force $\bbf_j$ that is the filtered version of the stiffness proportional viscous damping force $\mb{K}\dot{\bu}$ (c.f., \eqsref{eq:vel_formulation}).
...
From an implementation perspective, this operation might be mitigated by applying the reciprocal of the scaling factor to the rest components in the equation, namely matrices that already require scaling or vectors that are inexpensive to compute.
...
In addition, the non-standard quantity $\mb{K}\bv$ must be computed, assembled, and stored as an additional history variable, resulting in the highest computational cost among the three formulations although its analytical form is the most concise one.
...
Since $\mb{M}$ is typically sparser than $\mb{K}$ and must be scaled regardless in displacement based dynamic analyses, the additional computational cost is negligible.
