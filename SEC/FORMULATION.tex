...
Besides, for the system to be stable, it is required that $\Re\left(s_j\right)>0$.
...
\end{gather}
The (magnitude of, if a signed definition is used) loss ratio stays the same since the scalar coefficient is the reciprocal of that in the previous case.
...
It is in fact also equivalent to \eqsref{eq:u_right} after a simple mapping of parameters.
...
The non-trivial $\bh_j$, that is, $\by_j$, often would not need to be stored separately.
...
%                                                 &    0    &    1    & $\left(\alpha\mb{M}_0+\beta\mb{K}_0\right)\ba$ &      $\mb{0}$       & Rayleigh damping                               \\
\bottomrule
    \end{tabular}
\end{table}
