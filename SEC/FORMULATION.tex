...
Assuming that the contributions of $\bu$, $\bv=\dot{\bu}$, and $\ba=\ddot{\bu}$ are separable, without coupling among them, the resistance $\br$ can be divided into three components
\begin{gather}
    \br=\br_{\bu}+\br_{\bv}+\br_{\ba},
\end{gather}
where $\br_{\baleph}=\br_{\baleph}\left(\baleph\right)$ is a function of $\baleph$ and $\baleph\in\left\{\bu,\bv,\ba\right\}$.
...
To ensure stability, the poles $s_j$ must satisfy $\Re\left(s_j\right)>0$.
...
This equivalence may offer numerical advantages, as shown later, when the mass matrix is constant, as is often the case, and more sparse than the stiffness matrix, particularly under a lumped mass formulation.
...
However, $\mb{K}'$ fails to meet this requirement because it does not converge to the static stiffness $\mb{K}$ at low frequencies in \eqsref{eq:u_stiffnesses}.
...
From an implementation perspective, this operation can be mitigated by applying the reciprocal of the scaling factor to other components of the equation, namely matrices that already require scaling or vectors that are inexpensive to compute.
