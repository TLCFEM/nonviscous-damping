...
In the following, it is thus assumed $s_j\in\mathbb{C}$ and $m_j\in\mathbb{C}$ with this additional constraint implied.
...
For proper sets of $s_j$ and $m_j$, it is thus possible to find the corresponding sets $s'_j$ and $m'_j$ to convert between \eqsref{eq:acc_cha} and \eqsref{eq:disp_cha}.
...
\begin{gather}
    ...
    \end{array}\right.
\end{gather}
Assuming harmonic solutions for $\bu=\mb{U}\exp\left(i\omega{}t\right)$, $\bbf_j=\mb{F}_j\exp\left(i\omega{}t\right)$, $\bq_j=\mb{Q}_j\exp\left(i\omega{}t\right)$ and $\bp=\mb{P}\exp\left(i\omega{}t\right)$, the equation of motion can be expressed in the frequency domain as
...
\end{gather}
It is then straightforward to see that \eqsref{eq:forced_freq} is
\begin{multline}
...
\end{multline}
with the help of \eqsref{eq:rate_eqv}.
