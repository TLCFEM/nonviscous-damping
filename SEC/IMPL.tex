...
\begin{gather}\label{eq:eqv_sys}
    ...
    \end{array}\right.
\end{gather}
Because the external load may be augmented as in \eqsref{eq:a_final}, $\mb{z}$ is used here instead.
...
Assuming that the equation of motion is satisfied at $t_{n+1}$, which is valid for a wide range of time integration methods (including the well-known Newmark method), the second expression is
